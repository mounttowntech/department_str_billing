...
exports.createAuditLog = asyncHandler(async (req, res) => {
  ...
  success(res, "AuditLog created", data, 201);
});
exports.getAllAuditLog = asyncHandler(async (req, res) => {
  const filter = {};
  if (req.query.status) filter.status = req.query.status;
  if (req.query.store) filter.store = req.query.store;
  const data = await Model.find(filter).sort({ createdAt: -1 });
  success(res, "AuditLog list", data);
});
exports.getAuditLogById = asyncHandler(async (req, res) => {
  ...
  if (!data)
    return res
      .status(404)
      .json({ success: false, message: "AuditLog not found" });
  success(res, "AuditLog details", data);
});
exports.updateAuditLog = asyncHandler(async (req, res) => {
  ...
  if (!data)
    return res
      .status(404)
      .json({ success: false, message: "AuditLog not found" });
  success(res, "AuditLog updated", data);
});
